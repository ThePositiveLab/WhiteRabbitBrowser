...
using static Neo.UI.AsyncBoundObject;
...
namespace Neo.UI
{
    internal partial class MainForm : Form
    {
        ...
        public MainForm(XDocument xdoc = null)
        {
            ...
            //Setup Browser
            browser = new ChromiumWebBrowser("https://tpl-whiterabbit.azurewebsites.net/")
            {
                Dock = DockStyle.Fill
            };
            ...
            browser.RegisterAsyncJsObject("NEO", new AsyncBoundObject(), BindingOptions.DefaultBinder);
            txtBrowserURL.Text = "https://tpl-whiterabbit.azurewebsites.net/";
        ...
        {
            this.InvokeOnUiThreadIfRequired(() => txtBrowserURL.Text = e.Address);
        }
        ...
        {
            browser.Load(txtBrowserURL.Text);
        }
    }
    ...
    public class AsyncBoundObject
    {
        ...
        {
            string identity = "User not logged in";
            if (Program.CurrentWallet != null)
            {
                foreach (UInt160 scriptHash in Program.CurrentWallet.GetAddresses().ToArray())
                {
                    VerificationContract contract = Program.CurrentWallet.GetContract(scriptHash);
                    //only normal address, not contract
                    //get only first one
                    if (contract != null)
                    {
                        identity = Wallet.ToAddress(scriptHash);
                        break;
                    }
                }
        ...
        {
            string height = Blockchain.Default.HeaderHeight.ToString();
            return height;
        }

        public string TestMethod(string scriptHash, string method, string parameter)
        {
            return TestMethodWithODT(scriptHash, method, parameter, "string");
        }

        public string TestMethodWithODT(string scriptHash, string method, string parameter, string outputDataType)
        {
            UInt160 parsedHash;
            string script = "";

            if (!UInt160.TryParse(scriptHash, out parsedHash))
            {
                return "ERR";
            }


            var json = Newtonsoft.Json.Linq.JArray.Parse(parameter);

            #region Build script
            using (ScriptBuilder scriptBuilder = new ScriptBuilder())
            {
                List<ContractParameter> requiredParameters = new List<ContractParameter>();

                ContractParameter param = new ContractParameter(ContractParameterType.ByteArray);
                string s = method;
                param.Value = System.Text.ASCIIEncoding.Default.GetBytes(s);
                requiredParameters.Add(param);

                var array = new List<ContractParameter>();

                for (int i = 0; i < json.Count; i++)
                {
                    string hexValue = StringToHex(json[i].ToString());
                    //array.Add(new ContractParameter { Type = ContractParameterType.ByteArray, Value = System.Text.ASCIIEncoding.Default.GetBytes(json[i].ToString()) });

                    if ((method == "summon" && i != 0) || (method == "getmessage"))
                    {
                        array.Add(new ContractParameter { Type = ContractParameterType.Integer, Value = BigInteger.Parse(json[i].ToString()) });
                    }
                    else
                    {
                        array.Add(new ContractParameter { Type = ContractParameterType.ByteArray, Value = hexValue.HexToBytes() });
                    }
                }
                var param1 = new ContractParameter(ContractParameterType.Array);
                param1.Value = array;
                requiredParameters.Add(param1);

                //Somehow just need blank array
                var array2 = new List<ContractParameter>();
                var param2 = new ContractParameter(ContractParameterType.Array);
                param2.Value = array2;
                requiredParameters.Add(param2);

                PushParameters(scriptBuilder, requiredParameters);

                scriptBuilder.EmitAppCall(parsedHash.ToArray(), false);
                script = scriptBuilder.ToArray().ToHexString();
            }
            #endregion
        ...
        public string InvokeMethod(string scriptHash, string method, string parameter)
        {
            return InvokeMethod(scriptHash, method, parameter, "string");
        }

        public string InvokeMethod(string scriptHash, string method, string parameter, string outputDataType)
        {
            UInt160 parsedHash;
            string script = "";
            ...
            #region Sign & Broadcast
            Fixed8 fee = tx.Gas.Equals(Fixed8.Zero) ? Fixed8.FromDecimal(0.001m) : Fixed8.Zero;
            var transaction = Program.CurrentWallet.MakeTransaction(new InvocationTransaction
            {
                Version = tx.Version,
                Script = tx.Script,
                Gas = tx.Gas,
